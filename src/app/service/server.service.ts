...
export class ServerService {
  private readonly apiUrl = 'https://serverapp-backend.onrender.com';
  ...
  getServer$(): Observable<CustomResponse> {
    return this.http.get<CustomResponse>(`${this.apiUrl}/server/list`).pipe(
      tap(console.log),
          ...
          : {
              ...
              message:
                response.data.servers.filter(
                  ...
                  ? `Servers filtered by ${
                      status === Status.SERVER_UP ? 'SERVER_UP' : 'SERVER DOWN'
                    } status`
